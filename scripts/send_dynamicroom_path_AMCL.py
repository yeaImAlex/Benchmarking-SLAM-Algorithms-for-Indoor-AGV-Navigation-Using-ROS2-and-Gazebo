...
import math
import os
import time  # Essential for delays
...
def main():
    ...
    # We set this to (0,0) assuming that is your spawn point.
    # If your robot spawns elsewhere, update these coordinates.
    initial_pose = create_pose(navigator, 0.0, 0.0, 0.0)
    
    print("Setting initial pose...")
    navigator.setInitialPose(initial_pose)

    # --- CRITICAL WAIT: ALLOW AMCL TO INITIALIZE ---
    # This prevents the "Request Rejected" error by giving Nav2/AMCL 
    # time to process the initial pose and update the costmap.
    print("Waiting 5 seconds for AMCL to stabilize...")
    time.sleep(5.0)

    # --- 2. LOAD WAYPOINTS FROM YAML ---
    yaml_file_path = os.path.join(os.getcwd(), 'dynamic_room_waypoints.yaml')
    ...
    # --- CRITICAL FIX: CLEAR COSTMAPS ---
    # This removes "ghost obstacles" from the map that might exist 
    # before the robot starts moving.
    print("Clearing costmaps to remove ghost obstacles...")
    navigator.clearAllCostmaps()
    time.sleep(2.0) # Wait for clear to take effect
    ...
    while not navigator.isTaskComplete():
        ...
        if feedback:
             # Using carriage return \r to update the line instead of spamming
